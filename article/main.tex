...
\revision{
For segment 41, the value 1.0 at nonzero distance means that all 43 test samples belonging to this segment were assigned to another section. This makes segment 41 the main outlier in the topological-distance analysis. Segment 41 consists of a single pipe connecting a consumer to a branching node, resulting in fewer training examples compared to segments containing multiple pipes (N pipes x 225 defective samples per pipe). Since figure 5 aggregates errors only by topological distance, it indicates the local nature of the misclassifications but does not identify which classes are confused with each other. The corresponding ambiguity is therefore analyzed below using the class-distribution plots and confusion matrices.
}
...
% \revision{
The class-distribution panels in Figures~\ref{fig:supply_distribution_confusion} and~\ref{fig:return_distribution_confusion} show, for each network section, the number of true defect instances together with the counts of correctly and incorrectly predicted assignments. In these panels, ``True'' denotes the number of test
samples whose simulated defect is located in the corresponding section, while
\revision{``True Positives''} and \revision{``False Positives''} denote correct and incorrect
...
% \revision{
% The distribution of predicted classes and the corresponding confusion matrices
...
% separating weak fault signatures in the farthest part of the network. A
% secondary supply-side ambiguity is observed for segment 35: defects in this
% segment are sometimes assigned to the adjacent and larger segments 25 and 36,
% which can produce similar aggregated hydraulic and thermal signatures while
% also being represented by more training samples.

\revision{
The distribution of predicted classes and the corresponding confusion matrices in Figures~\ref{fig:supply_distribution_confusion} and~\ref{fig:return_distribution_confusion} provide more detailed information about the segment 41 outlier identified in Figures~\ref{fig:dist_cm} and~\ref{fig:metric_dist}. For the supply subnet, the main errors are concentrated in the local group of segments 32, 36, and 41. These segments are among the most remote supply-side segments from the heat source. These segments are among the farthest supply-side segments from the heat source. As a result, the thermohydraulic signature from these segments is attentuated, making defects in them less pronounced in the available observations. However, the outlier behaviour of segment 41 is not explained by distance from the heat source alone. Segments 37 and 38 are also remote, but their fault signatures remain distinguishable under the available sensor configuration.}

\revision{
A more specific explanation is the combination of attenuation, unequal class representation, and sparse observability. As noted above, segment 41 is a single-pipe class, whereas segments 32 and 36 include more pipes and therefore provide more training examples. In addition, segments 32, 36, and 41 form a T-shaped group that, in the supply subnet, is observed mainly through one downstream sensor at the end of segment 41. Faults in these neighbouring segments can therefore produce similar thermohydraulic signatures, causing the model to confuse them with each other. A secondary supply-side ambiguity is also observed for segment 35, whose defects are sometimes assigned to the adjacent and larger segments 25 and 36. This follows the same general pattern of local confusion between neighbouring sections, but it is less pronounced than the segment 41 case.}
