...
% Revision highlighting. Set \showrevisionsfalse for the final publication version.
\newif\ifshowrevisions
\showrevisionstrue
% \showrevisionsfalse

\newcommand{\revisionstyle}[1]{\textcolor{blue}{#1}}
\newcommand{\revision}[1]{\ifshowrevisions\revisionstyle{#1}\else#1\fi}
...
\corres{Correspondence: i.plokhikh@g.nsu.ru (I.P.), d.pushkarev@g.nsu.ru (D.P.)}
...
% Keywords
\keyword{district heating; graph neural networks; fault detection; fault localization; thermohydraulic simulation; network topology}
...
the detection and localization of faults in district heating
...
fault detection methodologies and their practical
...
% \revision{From an engineering perspective, the probability and
...
% does not replace these engineering and monitoring practices, but
...
options — effectively, the entire physical model must be reconstructed or recalibrated. This is a sufficiently complex and demanding process. Moreover,
...
% \revision{
Recent studies on water and gas distribution systems show why
...
\begin{figure}[H]
	...
	\subfloat[\centering]{\includegraphics[width=4.0cm]{Figures/k_0.png}}
	%\hfill
	\subfloat[\centering]{\includegraphics[width=4.0cm]{Figures/k_1.png}}
	\subfloat[\centering]{\includegraphics[width=4.0cm]{Figures/k_2.png}}
	%\hfill
	%\isPreprints{}{% This command is only used for ``preprints''.
	% \end{adjustwidth}
	%} % If the paper is ``preprints'', please uncomment this parenthesis.
...
% \revision{
Thus, the modeled object is a local hot-water supply/return
DHN segment rather than a complete city-wide heat-supply system. In this model consumer
...
% \revision{
Therefore, the boundary conditions in the generated dataset
are scenario-dependent values defined by this operating curve, where the ambient-temperature range specifies the load
...
% \revision{
increased heat transfer in the ranges 120--150\%, 150--170\%,
170--200\%, 200--300\%, and 300--400\% of the baseline $\alpha$ value.
% }

% \revision{
Consumer-modeling pipes are auxiliary graph edges that
...
% \revision{
In this formulation, a defect is not modeled as a mass leak:
...
\toprule
\textbf{Outdoor temp.} & \textbf{Subnet} & \textbf{Max / Min / $\Delta$ (°C)} \\
\midrule
\multirow{2}{*}{-34°C} & Supply & 177.30 / 170.36 / 6.94 \\
                       & Return & 129.07 / 125.93 / 3.14 \\
\midrule
\multirow{2}{*}{+10°C} & Supply & 64.45 / 63.94 / 0.51 \\
                       & Return & 57.46 / 56.90 / 0.56 \\
\bottomrule
\end{tabularx}
\caption{Supply and return water temperatures at the heat source for the minimum and maximum outdoor temperature scenarios.}\label{tab:temperatures}
\end{table}

\revision{
The corresponding supply and return water temperatures at the heat source for the extreme scenarios are summarized in Table~\ref{tab:temperatures}. The temperature difference across the network is noticeably larger in the supply subnet than in the return subnet at lower outdoor temperatures, whereas at higher temperatures the thermal losses are less pronounced.
}
...
localization in the DHN. 
% \revision{
To mimic realistic sparse monitoring,
	...
	of the 14 consumers (2 nodes per consumer).
    % }
    This configuration mirrors real-world
...
\begin{figure}[H]
	...
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		\includegraphics[width=16.0 cm]{Figures/dhn_example.png}
	\end{adjustwidth}
	\caption{An illustration of the district heating network. The scale bar corresponds to 50 m.\label{fig:dhn}}
\end{figure}
...
70:15:15. 
% \revision{
We conducted 3-fold cross-validation, obtaining
    ...
    % }
% \revision{
The split was performed randomly over generated operational
...
\begin{figure}[H]
	\centering
	\subfloat[\centering Cross-entropy loss]{\includegraphics[width=0.48\textwidth]{Figures/Loss_combined.png}}
	\hfill
	\subfloat[\centering Accuracy]{\includegraphics[width=0.48\textwidth]{Figures/Accuracy_combined.png}}
...
effectively handling the classification task. 
% \revision{
For a given
	class, precision is the fraction of predictions assigned to the class that
	are correct, while recall is the fraction of true samples of the class that
...
\begin{figure}[H]
	...
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		%} % If the paper is ``preprints'', please uncomment this parenthesis.
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/fwd_result_distance_confusion_matrix_transposed.png}}
		%\hfill
		\\
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/bwd_result_distance_confusion_matrix_transposed.png}}
		%\hfill
		%\isPreprints{}{% This command is only used for ``preprints''.
	\end{adjustwidth}
	%} % If the paper is ``preprints'', please uncomment this parenthesis.
...
% \revision{
For segment 41, the value 1.0 at nonzero distance means that all 43 test samples belonging to this segment were assigned to another section. This makes segment 41 the main outlier in the topological-distance analysis. Segment 41 consists of a single pipe connecting a consumer to a branching node, resulting in fewer training examples compared to segments containing multiple pipes ($N$ pipes $\times$ 225 defective samples per pipe). Since Figure~\ref{fig:dist_cm} aggregates errors only by topological distance, it indicates the local nature of the misclassifications but does not identify which classes are confused with each other. The corresponding ambiguity is therefore analyzed below using the class-distribution plots and confusion matrices.
% }
...
the vicinity of the actual fault.
% \revision{
...
% \revision{
To complement the hop-based analysis, we also evaluated the
...
\begin{figure}[H]
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/fwd_result_metric_distance_scatter.png}}
		\\
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/bwd_result_metric_distance_scatter.png}}
	\end{adjustwidth}
...
\begin{figure}[H]
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		\subfloat[\centering Class distribution]{\includegraphics[width=15.5cm]{Figures/fwd_result_class_distribution_split.png}}
		\\
		\subfloat[\centering Normalized confusion matrix]{\includegraphics[width=15.5cm]{Figures/fwd_result_confusion_matrix_normalized.png}}
	\end{adjustwidth}
...
\begin{figure}[H]
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		\subfloat[\centering Class distribution]{\includegraphics[width=15.5cm]{Figures/bwd_result_class_distribution_split.png}}
		\\
		\subfloat[\centering Normalized confusion matrix]{\includegraphics[width=15.5cm]{Figures/bwd_result_confusion_matrix_normalized.png}}
	\end{adjustwidth}
...
% \revision{
The class-distribution panels in Figures~\ref{fig:supply_distribution_confusion} and~\ref{fig:return_distribution_confusion} show, for each network section, the number of true defect instances together with the counts of correctly and incorrectly predicted assignments. In these panels, ``Ground Truth'' denotes the number of test
samples whose simulated defect is located in the corresponding section, while
``True Positives'' and ``False Positives'' denote correct and incorrect
...
% \revision{
The distribution of predicted classes and the corresponding confusion matrices in Figures~\ref{fig:supply_distribution_confusion} and~\ref{fig:return_distribution_confusion} provide more detailed information about the segment 41 outlier identified in Figures~\ref{fig:dist_cm} and~\ref{fig:metric_dist}. For the supply subnet, the main errors are concentrated in the local group of segments 32, 36, and 41, which are among the farthest supply-side segments from the heat source. As a result, the thermohydraulic signatures from these segments are attenuated, making defects in them less pronounced in the available observations. However, the outlier behaviour of segment 41 is not explained by distance from the heat source alone. Segments 37 and 38 are also remote, but their fault signatures remain distinguishable under the available sensor configuration.
% }

% \revision{
A more specific explanation is the combination of attenuation, unequal class representation, and sparse observability. The dataset is generated uniformly at the pipe level, but after pipes are grouped into section labels, longer sections naturally contribute more defective samples than shorter sections. Thus, segment 41, which contains a single pipe, provides fewer training examples than segments 32 and 36, which include more pipes. In addition, segments 32, 36, and 41 form a T-shaped group that, in the supply subnet, is observed mainly through one downstream sensor at the end of segment 41. Faults in these neighbouring segments can therefore produce similar thermohydraulic signatures, causing the model to confuse them with each other.
% }

% \revision{
The asymmetry between the supply and return predictions for segment 41 further supports this interpretation. In the return subnet, the flow direction is reversed, and additional downstream sensor information is available along the return path. This makes disturbances originating in segments 32 and 36 more directly observable than in the supply subnet, where only the sensor at the end of segment 41 lies downstream of this group. This particular combination of segment grouping, flow direction, and sensor placement is specific to segment 41 within the considered network topology, which supports the proposed explanation for the errors observed in this segment. Thus, the segment 41 error may be interpreted as a localized ambiguity within the considered sparse monitoring scheme rather than a general model weakness.
% }

% \revision{
For the return subnet more broadly, the misclassifications are more diffuse and scattered across several classes. This pattern is consistent with the narrower range of dynamic temperature and pressure values in the return line, where class-specific fault signatures become less pronounced relative to background variability. In particular, segment 0 is a long terminal section composed of five pipes and is often assigned when the true defect is located in the shorter neighbouring segments 1, 2, or 4. This effect is much weaker in the supply subnet, where stronger thermal and hydraulic gradients and additional downstream consumer measurements make the corresponding signatures more separable.
% }

% \revision{
Other local return-side ambiguities follow the same pattern but are less severe. Segment 35 contains three pipes and is sometimes confused with larger adjacent sections, namely segment 25 downstream and segment 36 nearby, whereas segments 25 and 36 themselves are recognized more reliably, with only minor misassignments to neighbouring classes. Segment 19 is also occasionally assigned to downstream segment 10, while segment 10 remains mostly distinguishable with only small local confusions. In the supply subnet, the confusion between segments 30 and 31 is limited; these sections are located near two consumer branches, so the available measurements provide more information for separating their fault signatures.
% }

% \revision{
The observed error pattern may also be informative from a monitoring design perspective. The high overall accuracy and local structure of the errors indicate that useful localization can be achieved with a limited set of suitably placed sensors, which is consistent with the optimal sensor placement concept discussed by Rosich et al.~\cite{rosich2012optimal}. In the present experiments, nonzero pressure and temperature measurements were retained only at the heat-source supply/return boundary nodes and at consumer connection nodes. At the same time, the largest localization errors measured along the pipe network are concentrated in the same local region where the confusion matrices indicate ambiguity between sections 32, 36, and 41. This suggests that the present sensor layout provides insufficient observability for this particular T-shaped group. Therefore, pipe-junction nodes incident to these sections are natural candidate locations for additional measurement points.
% }

% \revision{
More generally, a calibrated thermohydraulic model and a trained fault localization model can be used as a heuristic approach for sparse sensor augmentation. Since simulated states are available for the full network before sensor masking, candidate sensor layouts can be emulated by retaining measurements only at selected nodes, including intermediate junction nodes that are not instrumented in the current sparse layout. These layouts can then be compared by checking whether recurring confusions between neighbouring sections decrease. If the same local group of sections remains difficult to separate, it can be treated as insufficiently observable and considered for the placement of additional sensors. This procedure does not replace formal optimal sensor placement, but it can provide a simulation-based supporting tool for testing candidate layouts and selecting promising sensor locations under limits on the number of sensors.
% }
...
% \revision{
The distance confusion matrix reveals that most errors are localized within a topological distance of 1-2 sections from the actual defect, indicating that the model captures the attenuation of hydraulic and thermal disturbances along the network topology while still exhibiting local ambiguities in poorly observed regions.
% }
% \revision{
...
% \revision{
In practical predictive-maintenance workflows, the proposed
...
% \revision{
The error analysis also suggests an additional use of the proposed framework for sensor-layout assessment. Although most misclassifications remain topologically local, the recurring confusion between sections 32, 36, and 41 indicates a local observability limitation under the current sparse monitoring scheme. Such persistent confusion patterns can help identify candidate locations for additional sensors and provide a simulation-based approach for future sensor-placement studies.
% }
