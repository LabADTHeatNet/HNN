...
the detection and localization of leaks in district heating pipelines is a key
operational priority. This challenge, however, is not unique to district
heating systems.
...
Studies on water \cite{wu2024feel, zhang2025algorithm, li2024convolutional,
	csahin2023prediction, truong2024graph} and gas \cite{zhang2023towards}
distribution systems have demonstrated that GNN-based methods achieve superior
fault localization accuracy compared to topology-agnostic approaches, precisely
because they exploit the structural constraints that dictate how anomalies
propagate through interconnected network components. Specific results include
leakage detection accuracy above 90\%, topology-aware pressure reconstruction
with significantly lower error compared to previously used methods such as SVM
(\c{S}ahin \& Y{\"u}ce, 2023), Truong et al. reported a $\approx 40\%$
reduction in pressure estimation error using a GNN-based model. For gas network
state estimation, Li \& Wu (2024) proposed a CGNN (Composite Graph Neural
Network) architecture that fuses segment-level features, reporting accuracies
of $90.28\%$ and $>85\%$ in dense and sparse monitoring zones, respectively.
Inspired by these findings, this paper investigates the applicability of GNNs
to DHNs, aiming to adapt these topology-aware architectures to the specific
challenges of heat distribution monitoring.
...
pipes. Five defect severity levels were considered, corresponding to 120--400\%
of the baseline $\alpha$ value.
...
localization in the DHN. \textcolor{blue}{To mimic realistic sparse monitoring,
	dynamic pressure and temperature values are retained only at 30 nodes: the heat
	source inlet/outlet, the heat sink, and the supply/return connection points of
	the 14 consumers (2 nodes per consumer).} This configuration mirrors real-world
...
The model was optimized using Adam. The initial learning rate was set to
...
effectively handling the classification task. \textcolor{blue}{The F1-score is
	the harmonic mean of precision and recall, while macro F1 is the unweighted
...
\textcolor{blue}{Segment 41 consists of a single pipe connecting a consumer to a branching node, resulting in fewer training examples compared to segments containing multiple pipes (N pipes \texttimes{} 225 defective samples per pipe). Additionally, segment 41 is topologically the farthest from the heat source among supply-side segments; temperature and pressure signals are attenuated, making the defect less pronounced. Consequently, the model often misclassifies faults from segment 41 as belonging to neighbouring segments 32 or 36, each of which comprises more pipes.}
...
\begin{figure}[H]
	\begin{adjustwidth}{-\extralength}{0cm}
		\centering
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/fwd_result_class_distribution_split.png}}
		% \hfill
		\\
		\subfloat[\centering]{\includegraphics[width=16.0cm]{Figures/bwd_result_class_distribution_split.png}}
	\end{adjustwidth}
	\caption{Distribution of predicted classes for (\textbf{a}) supply and (\textbf{b}) return subnets, excluding "no defect" class.
		\label{fig:pred_distribution}}
\end{figure}

\begin{figure}[H]
	\centering
	\subfloat[\centering]{\includegraphics[width=10.0cm]{Figures/fwd_result_confusion_matrix.png}}
	% \hfill
	\\
	\subfloat[\centering]{\includegraphics[width=10.0cm]{Figures/bwd_result_confusion_matrix.png}}
	\caption{Confusion matrices for (\textbf{a}) supply and (\textbf{b}) return subnets, excluding "no defect" class.
		\label{fig:confusion_matrix}}
\end{figure}

\textcolor{blue}{The distribution of predicted classes and the corresponding confusion matrices
on Figures~\ref{fig:pred_distribution} and \ref{fig:confusion_matrix} support
the earlier observations. For the supply subnet, almost all errors are concentrated
on segment 41. For the return subnet, the
...
discriminative, and the noise begins to mask the underlying data patterns.}
...
different district heating geometries, while addressing the inevitable domain
shift between simulated training regimes and real-world seasonal and load
variations. However, we recognize that the most significant challenge remains
the acquisition of high-quality, labeled data to support such data-driven
methodologies.
