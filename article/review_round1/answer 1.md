...
Response 2: We agree. We added non-GNN baselines to Table 2: Random Forest, Histogram Gradient Boosting, and MLP. The proposed GNN outperforms all these topology-agnostic models, especially on the return subnet and Macro F1. We did not include a physics-based residual-localization baseline because the present dataset does not model actual mass leakage or measured field residuals; this limitation is now clarified in the dataset description. (lines 285-290, lines 389-399, Table 2).
...
Response 6: We redrew and enlarged Figure 2 using the updated network visualization. The revised figure uses a larger layout and clearer labels/legend for the DHN topology. (Figure 2).
