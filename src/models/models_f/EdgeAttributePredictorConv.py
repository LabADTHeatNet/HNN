...
class EdgeAttributePredictorConv(nn.Module):
    def __init__(self, in_channels, out_channels, fc_channels, conv_channels,
        ...
        for idx in range(len(conv_channels)):
            ...
            # self.conv_net.append(gnn.GCNConv(in_c, out_c))
            # self.conv_net.append(gnn.SAGEConv(in_c, out_c, aggr='max'))
            self.conv_net.append(gnn.GATv2Conv(in_c, out_c, **gat_args))
