...
class EdgeRegressionNetSAGE(torch.nn.Module):
    def __init__(self, node_in_dim, edge_in_dim, hidden_dim, out_dim, aggr='mean', **kwargs):
        super(EdgeRegressionNetSAGE, self).__init__()

        # GCN для обработки node features
        self.node_conv1 = SAGEConv(node_in_dim, hidden_dim, aggr=aggr)
        self.node_conv2 = SAGEConv(hidden_dim, hidden_dim, aggr=aggr)
